...
void hash_MD5(unsigned char *in, unsigned int len, unsigned char *out)
{
    MD5_CTX mdContext;
	int i;

    MD5Init (&mdContext);
    MD5Update (&mdContext, in, len);
    MD5Final (&mdContext);

	for (i = 0; i < 16; i++)  out[i] = mdContext.digest[i];
